...
from config import API_ID, API_HASH, FREEMIUM_LIMIT, PREMIUM_LIMIT, OWNER_ID, DEFAULT_SESSION, UPDATES
...
async def process_special_links(userbot, user_id, msg, link):
    if userbot is None:
        # Send a new message instead of editing, and ensure it persists
        error_msg = await app.send_message(
            user_id,
            "Try logging in to the bot and try again."
        )
        await asyncio.sleep(30)  # Keep the message for 30 seconds
        try:
...
async def batch_link(_, message):
    ...
    join_button = InlineKeyboardButton("⚡ Updates ⚡", url=f"https://t.me/{UPDATES}")
    keyboard = InlineKeyboardMarkup([[join_button]])
    pin_msg = await app.send_message(
        user_id,
        f"**Batch process started**\n\n⚡ __Processing:__ `0/{cl}`",
    ...
    try:
        ...
        if normal_links_handled:
            ...
            await pin_msg.edit_text(
                f"Batch completed successfully for {cl} messages 🥳, you can add your next batch task now! 😉",
                reply_markup=keyboard
            )
